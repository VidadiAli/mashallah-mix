export const language = {
    az: {
        menu: {
            dashboard: 'Ana Səhifə',
            artists: 'Sənətçilər',
            myLists: 'Playlistlərim',
            likeds: 'Bəyəndiklərim'
        },
        user: {
            main: {
                head: 'Trend Musiqlər',
                headSpan: 'Daha çoxunu burada kəşf et'
            },
            likeds: {
                head: 'Bəyəndiklərim'
            }
        },
        footer: {
            title: '🎵 MM',
            description: 'Musiqi dünyasını kəşf et, sevdiyin mahnıları dinlə və öz playlistlərini yarat.',
            socials: 'Sosial Şəbəkələr',
            copyright: '© 2026 Mashallah Mix. Bütün hüquqlar qorunur.'
        },
        playList: {
            msg: {
                successCreate: 'Siyahı uğurla yaradıldı',
                errorExsistName: 'Bu ad playlistlərinizdə mövcuddur. Unikal ad fikirləşin',
                successAdding: 'Uğurla siyahıya əlavə olundu',
                errorExsistInList: 'Playlistdə mövcuddur',
                errorBlankField: 'Ad boş qala bilməz'
            },
            add: 'Playlistə əlavə et:',
            create: 'Yeni siyahı yarat',
            createBtn: 'Yarat',
            traks: 'parça',
            noList: 'Playlist mövcud deyil',
            playListsHead: 'Playlistlərim',
            playListsText: 'Musiqi kolleksiyan',
            noPlaylistsTitle: 'Hələ playlist yoxdur',
            noPlaylistsText: 'İlk playlistinizi yaradın və sevdiyiniz mahnıları toplayın.'
        },
        more: 'Daha çox',
        logo: 'Premium Musiqi Platforması',
        currentPlayList: 'Bu playlisti dinlə',
        playListResponse: 'Playlisti az əvvəl dəyişdiniz. Yenisi ilə davam edəcəksiz',
        artist: {
            Artist: 'Sənətçi',
        }
    },

    eng: {
        menu: {
            dashboard: 'Dashboard',
            artists: 'Artists',
            myLists: 'My Playlists',
            likeds: 'Likeds'
        },
        user: {
            main: {
                head: 'Trending Music',
                headSpan: 'Discover the most popular tracks'
            },
            likeds: {
                head: 'Liked Songs'
            }
        },
        footer: {
            title: '🎵 MM',
            description: 'Discover the world of music, listen to your favorite songs and create your own playlists.',
            socials: 'Social Media',
            copyright: '© 2026 Mashallah Mix. All rights reserved.'
        },
        playList: {
            msg: {
                successCreate: 'Playlist created successfully',
                errorExsistName: 'A playlist with this name already exists. Please choose a unique name.',
                successAdding: 'Added to the playlist successfully',
                errorExsistInList: 'Already exists in this playlist',
                errorBlankField: 'The name cannot be empty'
            },
            add: 'Add to playlist:',
            create: 'Create New Playlist',
            createBtn: 'Create',
            traks: 'tracks',
            noList: 'No playlists available',
            playListsHead: 'My Playlists',
            playListsText: 'Your own music collections',
            noPlaylistsTitle: 'No playlists yet',
            noPlaylistsText: 'Create your first playlist and collect your favourite songs.'
        },
        more: 'More',
        logo: 'Premium Music Platform',
        currentPlayList: 'Listen to this playlist',
        playListResponse: 'You recently changed the playlist. Playback will continue with the new one.',
        artist: {
            Artist: "Artist"
        }
    },

    ru: {
        menu: {
            dashboard: 'Панель управления',
            artists: 'Исполнители',
            myLists: 'Мои плейлисты',
            likeds: 'Понравившиеся'
        },
        user: {
            main: {
                head: 'Популярная музыка',
                headSpan: 'Откройте для себя самые популярные треки'
            },
            likeds: {
                head: 'Любимые песни'
            }
        },
        footer: {
            title: '🎵 MM',
            description: 'Откройте мир музыки, слушайте любимые песни и создавайте свои плейлисты.',
            socials: 'Социальные сети',
            copyright: '© 2026 Mashallah Mix. Все права защищены.'
        },
        playList: {
            msg: {
                successCreate: 'Плейлист успешно создан',
                errorExsistName: 'Плейлист с таким названием уже существует. Придумайте уникальное название.',
                successAdding: 'Успешно добавлено в плейлист',
                errorExsistInList: 'Уже есть в этом плейлисте',
                errorBlankField: 'Название не может быть пустым'
            },
            add: 'Добавить в плейлист:',
            create: 'Создать новый плейлист',
            createBtn: 'Создать',
            traks: 'треки',
            noList: 'Плейлисты отсутствуют',
            playListsHead: 'Мои плейлисты',
            playListsText: 'Ваши собственные музыкальные коллекции',
            noPlaylistsTitle: 'Плейлистов пока нет',
            noPlaylistsText: 'Создайте свой первый плейлист и соберите любимые песни.'
        },
        more: 'Ещё',
        logo: 'Премиальная музыкальная платформа',
        currentPlayList: 'Слушать этот плейлист',
        playListResponse: 'Вы недавно сменили плейлист. Воспроизведение продолжится с новым.',
        artist: {
            Artist: "Исполнитель"
        }
    },

    tr: {
        menu: {
            dashboard: 'Gösterge Paneli',
            artists: 'Sanatçılar',
            myLists: 'Çalma Listelerim',
            likeds: 'Beğenilenler'
        },
        user: {
            main: {
                head: 'Trend Müzik',
                headSpan: 'En popüler şarkıları keşfedin'
            },
            likeds: {
                head: 'Beğendiğim Şarkılar'
            }
        },
        footer: {
            title: '🎵 MM',
            description: 'Müzik dünyasını keşfedin, favori şarkılarınızı dinleyin ve kendi çalma listelerinizi oluşturun.',
            socials: 'Sosyal Medya',
            copyright: '© 2026 Mashallah Mix. Tüm hakları saklıdır.'
        },
        playList: {
            msg: {
                successCreate: 'Çalma listesi başarıyla oluşturuldu',
                errorExsistName: 'Bu isimde bir çalma listesi zaten mevcut. Lütfen benzersiz bir isim seçin.',
                successAdding: 'Çalma listesine başarıyla eklendi',
                errorExsistInList: 'Bu çalma listesinde zaten mevcut',
                errorBlankField: 'İsim boş bırakılamaz'
            },
            add: 'Çalma listesine ekle:',
            create: 'Yeni Çalma Listesi Oluştur',
            createBtn: 'Oluştur',
            traks: 'parça',
            noList: 'Çalma listesi bulunmuyor',
            playListsHead: 'Çalma Listelerim',
            playListsText: 'Kendi müzik koleksiyonların',
            noPlaylistsTitle: 'Henüz çalma listesi yok',
            noPlaylistsText: 'İlk çalma listenizi oluşturun ve sevdiğiniz şarkıları bir araya getirin.'
        },
        more: 'Daha Fazla',
        logo: 'Premium Müzik Platformu',
        currentPlayList: 'Bu çalma listesini dinle',
        playListResponse: 'Çalma listesini az önce değiştirdiniz. Oynatma yeni listeyle devam edecek.',
        artist: {
            Artist: "Sanatçı"
        }
    }
}